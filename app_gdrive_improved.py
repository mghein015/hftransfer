...
def _gdrive_resolve_parent(
    ...
    """
    Ensure intermediate Drive folders exist and return (parent_id, filename).

    folder_cache avoids repeating Drive list calls for directories that were
    already resolved during this transfer. Cache keys include the root folder,
    so the same cache cannot accidentally mix two Drive destinations.
    """
    ...
    for part in parts[:-1]:
        relative_folder = f"{relative_folder}/{part}" if relative_folder else part
        cache_key = (root_folder_id, relative_folder)

        cached_id = folder_cache.get(cache_key) if folder_cache is not None else None
        if cached_id:
            current = cached_id
            continue
        ...
        if folder_cache is not None:
            folder_cache[cache_key] = current
...
def _gdrive_upload_file_resumable(
    ...
    """
    Upload one file using a resumable Drive upload.

    The caller supplies existing_id from its single pre-download existence
    check. This prevents a second Drive lookup while retaining safe skip or
    overwrite behavior.
    """
...
def _bucket_stream_sync(repo_id: str, path: str, token: str, filename: str, range_hdr: Optional[str], inline: bool = False):
    ...
    url = f"hf://buckets/{repo_id}/{path.lstrip('/')}"
...
class HFManager:
    ...
    def transfer_to_gdrive_sync(self, job_id: Optional[str], req: "GDriveTransferRequest") -> dict:
        ...
        # Cache resolved Drive folders for this job. This is especially useful
        # when many files share the same directory tree.
        folder_cache = {}
        ...
        try:
            for idx, file_info in enumerate(all_files):
                ...
                local_path = None
                try:
                    # ── Resolve Drive parent folder FIRST ──────────────────
                    # This only creates intermediate Drive folders (cheap
                    # metadata calls) and does NOT touch the HF source, so
                    # it's safe to do before downloading anything.
                    drive_rel = rel_path
                    if req.path_filter and req.path_filter.strip() not in ("", "*"):
                        prefix = req.path_filter.rstrip("/") + "/"
                    ...
                    parent_id, filename = _gdrive_resolve_parent(
                        service, drive_rel, req.gdrive_folder_id, folder_cache
                    )

                    # ── Single existence check BEFORE download ─────────────
                    # This check is retained so existing files are skipped
                    # without downloading them. Its result is passed into the
                    # uploader, avoiding a duplicate Drive API request.
                    existing_id = _gdrive_file_exists(service, filename, parent_id)
                    if existing_id and not req.overwrite:
                        skipped_existing += 1
                        self._db(
                            job_id, progress=pct, status="uploading",
                            error_message=f"[{idx + 1}/{total}] {rel_path} (already on Drive, skipped)",
                        )
                        continue
                    ...
                    if req.repo_type == "bucket":
                        ...
                        with self.src_fs.open(f"hf://buckets/{req.repo_id}/{rel_path}", "rb") as f_in:
        ...
        async def _run_gdrive_job():
            ...
            try:
                ...
                summary = (
                    ...
                    + (f", {result.get('skipped_existing', 0)} already on Drive" if result.get("skipped_existing") else "")
