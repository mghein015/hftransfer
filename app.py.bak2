        ...
        try:
            for idx, file_info in enumerate(all_files):
                ...
                try:
                    ...
                    # ── Skip-existing check BEFORE download ────────────────
                    # If overwrite is off and the file is already on Drive,
                    # bail out here so we never pull it from HF at all.
                    ...
                    # We already know from the check above that this path is
                    # either overwrite=True or genuinely new, so this call's
                    # own internal exists-check is just a cheap confirmation.
